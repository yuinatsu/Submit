...
VSOutput main(VSInput input)
{
	...
	// 4.ビュー座標を射影座標に変換
	output.svpos.xyz = mul(viewPos, g_base.projMat);
	...
	float3 worldBin;
	worldBin.x = dot(input.binorm, localWorldMat[0].xyz);
	worldBin.y = dot(input.binorm, localWorldMat[1].xyz);
	worldBin.z = dot(input.binorm, localWorldMat[2].xyz);
	float3 worldTan;
	worldTan.x = dot(input.tan, localWorldMat[0].xyz);
	worldTan.y = dot(input.tan, localWorldMat[1].xyz);
	worldTan.z = dot(input.tan, localWorldMat[2].xyz);
