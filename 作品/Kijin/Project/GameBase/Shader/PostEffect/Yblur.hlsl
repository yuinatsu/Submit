...
VSOutput main(VSInput input)
{
    ...
    output.tex0.xy = float2(0.0f, 1.0f / size.y);
    
    // 基準テクセルからV座標を+3テクセルずらすためのオフセットを計算する
    output.tex1.xy = float2(0.0f, 3.0f / size.y);
    
    // 基準テクセルからV座標を+5テクセルずらすためのオフセットを計算する
    output.tex2.xy = float2(0.0f, 5.0f / size.y);
    
    // 基準テクセルからV座標を+7テクセルずらすためのオフセットを計算する
    output.tex3.xy = float2(0.0f, 7.0f / size.y);
    
    // 基準テクセルからV座標を+9テクセルずらすためのオフセットを計算する
    output.tex4.xy = float2(0.0f, 9.0f / size.y);
    
    // 基準テクセルからV座標を+11テクセルずらすためのオフセットを計算する
    output.tex5.xy = float2(0.0f, 11.0f / size.y);
    
    // 基準テクセルからV座標を+13テクセルずらすためのオフセットを計算する
    output.tex6.xy = float2(0.0f, 13.0f / size.y);
    
    // 基準テクセルからV座標を+15テクセルずらすためのオフセットを計算する
    output.tex7.xy = float2(0.0f, 15.0f / size.y);
    ...
    output.tex0 += float4(uv, uv);
    output.tex1 += float4(uv, uv);
    output.tex2 += float4(uv, uv);
    output.tex3 += float4(uv, uv);
    output.tex4 += float4(uv, uv);
    output.tex5 += float4(uv, uv);
    output.tex6 += float4(uv, uv);
    output.tex7 += float4(uv, uv);
