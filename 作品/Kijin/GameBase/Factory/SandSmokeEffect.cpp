...
#include "../Component/Behavior/SandSmokeBehavior.h"
#include "../Component/Render/EffectRender.h"

SandSmokeEffect::SandSmokeEffect(ObjectManager& objectManager) :
	Factory{ objectManager }
{
}

ObjectID SandSmokeEffect::Create(ObjectID ownerID, const Vector3& pos, const Vector3& rot)
{
	...
	auto behavior = objectManager_.GetPool().Pop<SandSmokeBehavior>();
	...
	static_cast<Render&>(*modelRender).Load("Resource/Effect/SandSmokeStage1.efkefc");
...
void SandSmokeEffect::Destroy(ObjectID id)
{
	objectManager_.GetPool().Push(objectManager_.RemoveComponent<ObjectInfo>(id));
	objectManager_.GetPool().Push(objectManager_.RemoveComponent<SandSmokeBehavior>(id));
