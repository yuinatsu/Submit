...
float4 lerpColor(float fade, float4 color1, float4 color2)
{
    ...
    return lerp(color1_, color2, blend);
}
...
PSOutput main(PSInput input)
{
    PSOutput output;
    float fade;
    float4 color1, color2, texColor, skyColor, bokeColor;
    // 深度値を取得する
    ...
    bokeColor = boke.Sample(bokesam, input.uv);
    color1 = depthOfColor(fade, texColor, skyColor);
    color2 = lerpColor(fade, color1, bokeColor);
    output.color = color2;
    return output;
}
